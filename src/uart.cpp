...
void Uart::ITConfig(uint32_t it, FunctionalState enabled) { USART_ITConfig(_peripheral, it, enabled); }
...
uint16_t Uart::txSpaceAvailable(void) { return bufSize - txSpaceUsed() - 1; }
...
uint16_t Uart::rxWaiting(void) { return ((uint16_t)(rxTail - rxHead) % bufSize); }
...
#error
      ...
      (uint32_t)txBuf,
      0, // bufSize (CNDTR)
      DMA_DIR_PeripheralDST, DMA_PeripheralDataSize_Byte, DMA_MemoryDataSize_Byte, DMA_Mode_Normal, DMA_Priority_High,
...
void USART1_IRQHandler(void) { uart1._irqHandler(); }
#endif

#ifdef USE_USART_2

void USART2_IRQHandler(void) { uart2._irqHandler(); }
...
void USART3_IRQHandler(void) { uart3._irqHandler(); }
#endif
}
