...
int main() {
  ...
  AdcChannel *adcChan0 = adc1.addChannel(ADC_Channel_0);
  AdcChannel *adcChan1 = adc1.addChannel(ADC_Channel_1);
  AdcChannel *adcChan4 = adc1.addChannel(ADC_Channel_4);
  ...
  while (1) {
    adc1.waitConversion();
    printf("adc: %d\t%d\t%d\r\n", adcChan0->average, adcChan1->average, adcChan4->average);
    mDelay(adcChan0->average / 4);
    gpioLed.toggle();
  }
