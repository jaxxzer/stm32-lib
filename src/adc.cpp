...
#if defined(STM32F0)
void Adc::init(FunctionalState continuousConvMode, uint32_t resolution, uint32_t extTrigConvEdge, uint32_t extTrigConv,
               uint32_t dataAlign, uint32_t scanDirection) {
  // Configuration
...
#elif defined(STM32F1)
void Adc::init(FunctionalState continuousConvMode, uint32_t extTrigConv, uint32_t dataAlign) {
  // Configuration
  ADC_InitTypeDef _config;
  ADC_StructInit(&_config);
...
void Adc::_dmaConfig(void) {
  ...
  dma1c1.init((uint32_t) & (ADC1->DR), (uint32_t)_dmaBuf, _numChannels * _numSamples, DMA_DIR_PeripheralSRC,
              DMA_PeripheralDataSize_HalfWord, DMA_MemoryDataSize_HalfWord, DMA_Mode_Circular, DMA_Priority_High,
