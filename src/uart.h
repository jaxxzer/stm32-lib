...
class Uart {
public:
  Uart(USART_TypeDef *usartx) : _peripheral(usartx) { setClockEnabled(ENABLE); }
  ...
  void init(uint32_t baudrate, uint32_t stopbits = USART_StopBits_1, uint32_t wordlength = USART_WordLength_8b,
            uint32_t parity = USART_Parity_No, uint32_t mode = USART_Mode_Rx | USART_Mode_Tx,
  ...
  void setEnabled(FunctionalState enabled) { USART_Cmd(_peripheral, enabled); }
