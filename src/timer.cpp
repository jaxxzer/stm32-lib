...
void TimerChannelInput::init(uint16_t polarity, uint16_t filter, uint16_t prescaler, uint16_t selection) {
...
void Timer::setRepetitionCounter(uint16_t repetitions) { _peripheral->RCR = repetitions; }
...
void TimerChannelOutput::init(uint16_t ocMode, uint32_t pulse, uint16_t outputState, uint16_t outputNState,
                              uint16_t ocPolarity, uint16_t ocNPolarity, uint16_t ocIdleState, uint16_t ocNIdleState) {
...
void Timer::init(uint16_t prescaler, uint32_t period, uint16_t counterMode, uint16_t clockDivision,
...
uint32_t Timer::getAutoreload(void) { return _config.TIM_Period; }

void Timer::generateEvent(uint16_t TIM_EventSource) { TIM_GenerateEvent(_peripheral, TIM_EventSource); }
...
void Timer::setEnabled(FunctionalState enabled) { TIM_Cmd(_peripheral, enabled); }

void Timer::_initTimeBase(void) { TIM_TimeBaseInit(_peripheral, &_config); }

void Timer::_configBDTR() { TIM_BDTRConfig(_peripheral, &_config_bdtr); }
...
#if defined(STM32F0)
  uint32_t clk_f = RCC_ClocksStruct.PCLK_Frequency / (_config.TIM_Prescaler + 1);
#elif defined(STM32F1)
  uint32_t clk_f = RCC_ClocksStruct.PCLK1_Frequency / (_config.TIM_Prescaler + 1);
...
void Timer::_deleteCallbacks(void) { return; }
